 ...
 * @returns 菜单树响应
 */
export function getMenuTree(status?: number, roleIds?: number[]): Promise<MenuTreeResponse> {
  return requestGet<MenuTreeResponse>('/menu/tree', { status, roleIds }).then((res) => res.data)
}
